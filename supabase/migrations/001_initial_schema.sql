...
-- Qolify — Full database schema
-- Canonical source: MD files/SCHEMA.md
...
-- Model A only. Populated by bulk scraping pipeline.
...
CREATE TABLE composite_indicators (
  ...
  -- Indicator 8: Climate & Solar Score (expanded — D-021)
  climate_solar_score             DECIMAL(5,2),
  sunshine_hours_annual           INTEGER,
  sunshine_hours_monthly          DECIMAL(4,1)[],
  hdd_annual                      INTEGER,
  cdd_annual                      INTEGER,
  building_aspect                 TEXT,
  damp_risk_index                 DECIMAL(5,2),
  days_above_35c_annual           INTEGER,
  days_above_35c_trend            DECIMAL(5,2),
  heating_cost_annual_eur         INTEGER,
  cooling_cost_annual_eur         INTEGER,
...
-- Written by both Model A and Model B. Every analysis writes here.
...
CREATE INDEX vut_licences_region_idx ON vut_licences (region);
...
-- Climate normals per municipio (AEMET 1991–2020 + ERA5 gap-fill) — D-021
CREATE TABLE climate_data (
  id                          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  municipio_code              TEXT NOT NULL UNIQUE,
  municipio_name              TEXT,
  provincia                   TEXT,

  sunshine_hours_annual       INTEGER,
  sunshine_hours_jan          DECIMAL(4,1),
  ...
  rainfall_jul_mm             INTEGER,
  humidity_annual_pct         DECIMAL(4,1),
  humidity_winter_pct         DECIMAL(4,1),

  aemet_station_id            TEXT,
  era5_gap_fill               BOOLEAN DEFAULT FALSE,
...
-- PVGIS solar irradiance cache — queried per-analysis, cached at 0.01° resolution — D-021
...
-- Energy + EPC + solar constants — referenced by Indicator 1 and Indicator 8 (D-021)
CREATE TABLE eco_constants (
  id                        UUID PRIMARY KEY DEFAULT gen_random_uuid(),

  ecb_base_rate_pct         DECIMAL(4,3),
  typical_bank_spread_pct   DECIMAL(4,3),
  euribor_12m_pct           DECIMAL(4,3),

  gas_price_kwh_eur         DECIMAL(6,5),
  electricity_pvpc_kwh_eur  DECIMAL(6,5),
