...
async function calcDigitalViability(
  ...
  const has4G = row?.has_4g ?? has5G  // 5G cell implies 4G in reality
  const mobileClass = has5G ? '5G' : has4G ? '4G' : 'none'

  // Score: pick best of fibre / mobile fallback, then add cowork bonus (max +20).
  ...
  if (!fibreType || fibreType === 'none') {
    if (has5G) {
      alerts.push({ type: 'amber', category: 'connectivity', title: 'No fibre — 5G available',     description: '5G mobile broadband is the primary remote-work option here.' })
    } else if (has4G) {
      alerts.push({ type: 'amber', category: 'connectivity', title: 'No fibre — 4G only',          description: '4G is the only broadband option. Usable for most remote work but not streaming-heavy loads.' })
    } else {
      alerts.push({ type: 'red',   category: 'connectivity', title: 'No broadband coverage',       description: 'No registered fibre or mobile coverage for this property.' })
    }
